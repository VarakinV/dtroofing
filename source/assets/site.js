...
/* build Services dropdown into the nav (progressive enhancement) */
(function(){
  var links=document.getElementById('navLinks');
  if(!links) return;
  var svc=[].slice.call(links.querySelectorAll('a')).find(function(a){ return a.textContent.trim().toLowerCase()==='services'; });
  if(!svc) return;
  var items=[
    ['metal-roofing.html','Metal Roofing'],
    ['shingle-roofing.html','Shingle Roofing'],
    ['cedar-roofing.html','Cedar Roofing'],
    ['roof-repair.html','Roof Repair'],
    ['siding-eavestrough.html','Siding & Eavestrough'],
    ['insurance-claims.html','Insurance & Hail Claims']
  ];
  var dd=document.createElement('div'); dd.className='nav-dd';
  var toggle=document.createElement('a');
  toggle.href=svc.getAttribute('href'); toggle.className='nav-dd-toggle';
  toggle.innerHTML='Services <svg class="dd-caret" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.4" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M6 9l6 6 6-6"/></svg>';
  var menu=document.createElement('div'); menu.className='nav-dd-menu';
  items.forEach(function(it){ var a=document.createElement('a'); a.href=it[0]; a.textContent=it[1]; menu.appendChild(a); });
  dd.appendChild(toggle); dd.appendChild(menu);
  /* on mobile the toggle expands the submenu in place instead of navigating */
  ...
  svc.parentNode.replaceChild(dd, svc);
})();
...
(function(){
  ...
  if(t&&l){
    ...
    l.querySelectorAll('a').forEach(function(a){
      if(a.classList.contains('nav-dd-toggle')) return; /* toggle expands submenu, doesn't close menu */
